  ...
  const handleBlur = (e) => {
    ...
    if (e.target.name === "password") {
      ...
      if (isValid) {
        ...
        const validation = { ...validationMsg };
        validation.msg =
          "Password should be at least of 6 characters including a number";
        validation.errorStatus = true;
        setValidationMsg(validation);
      }
    }
  ...
  return (
    <div className="mx-auto">
      <form
        ...
        className="mx-auto m-4 p-4 shadow-lg rounded"
      >
        ...
        <input
          ...
          required
        />
        {validationMsg.errorStatus && (
          <p className="text-danger py-2">{validationMsg.msg}</p>
        )}
        <input
          type="submit"
          value={showSignIn ? "Register" : "Sign in"}
          className="btn btn-success px-4 py-1"
        />
        <p className="text-center py-3 text-secondary">
          {showSignIn ? "Already have an account?" : "Create a new account"}
          <input
            type="checkbox"
            name="signin"
            id="signin"
            ...
            className="d-none"
          />
          <label htmlFor="signin" className="mx-1 text-primary sign-in-toggle">
      ...
      <div className="messages w-100 text-center">
        {user.errorStatus && (
          <p className="text-danger border d-inline-block border-danger px-5 py-2 my-2">
            {user.errorMessage}
          </p>
        )}
        {user.isSignedIn && (
          <p className="text-success d-inline-block border border-success px-5 py-2 my-2">
      ...
      <div className="w-100 text-center">
        <button
          className="btn btn-info py-2 px-5 my-2"
